...
int lenseq_f_remap (gsl_vector * xx, void *par, gsl_vector * f)
{
  ...
  if ((x1 >= dims1[0]) || (x2 >= dims1[1]) || (x1 < 0) || (x2 < 0) ||
  	  (x1 >= dims2[0]) || (x2 >= dims2[1]) || (x1 < 0) || (x2 < 0)){ 
    ...
    def1i = interp_pix(alpha1data, x1, x2, dims1[0], dims1[1]);  
    def2i = interp_pix(alpha2data, x1, x2, dims2[0], dims2[1]);
  
    y1[0]=x1 - def1i; // pixels
    y1[1]=x2 - def2i; // pixels
    
    for (j=0;j<2;j++){
    	zero[j] = y1[j] - ysource[j];
    }
