...
void free_arrays_gridremap(void){
  ...
  free(pgridremap.kappacomp);  
  free(pgridremap.gamma1comp);  
  free(pgridremap.gamma2comp); 
  free(pgridremap.kappagrid); 
  free(pgridremap.psigrid); 
  free(pgridremap.psicomp); 
  free(pgridremap.gamma1grid); 
  free(pgridremap.gamma2grid);
...
void assign_arrays_gridremap(void){
  ...
  if ((pgridremap.psicomp = 
       ((double *)malloc(ngrid* ngrid*sizeof(double)))) == NULL) 
    error("malloc", "out of memory psicomp");
  cleanDoubleVar(ngrid* ngrid, pgridremap.psicomp);

  if ((pgridremap.kappacomp = 
       ((double *)malloc(ngrid* ngrid*sizeof(double)))) == NULL) 
    error("malloc", "out of memory kappacomp");
  cleanDoubleVar(ngrid* ngrid, pgridremap.kappacomp);

  if ((pgridremap.gamma1comp =
       ((double *)malloc(ngrid* ngrid*sizeof(double)))) == NULL) 
    error("malloc", "out of memory gamma1comp");
  cleanDoubleVar( ngrid* ngrid, pgridremap.gamma1comp);
  
  if ((pgridremap.gamma2comp =
       ((double *)malloc(ngrid* ngrid*sizeof(double)))) == NULL) 
    error("malloc", "out of memory gamma2comp");
  cleanDoubleVar( ngrid* ngrid, pgridremap.gamma2comp);  
  ...
  if ((pgridremap.psigrid = 
       ((double *)malloc(ngrid* ngrid*sizeof(double)))) == NULL) 
    error("malloc", "out of memory psigrid");
  cleanDoubleVar(ngrid* ngrid, pgridremap.psigrid);

  if ((pgridremap.kappagrid = 
       ((double *)malloc(ngrid* ngrid*sizeof(double)))) == NULL) 
    error("malloc", "out of memory kappagrid");
  cleanDoubleVar(ngrid* ngrid, pgridremap.kappagrid);

  if ((pgridremap.gamma1grid =
       ((double *)malloc(ngrid* ngrid*sizeof(double)))) == NULL) 
    error("malloc", "out of memory gamma1grid");
  cleanDoubleVar( ngrid* ngrid, pgridremap.gamma1grid);
  
  if ((pgridremap.gamma2grid =
       ((double *)malloc(ngrid* ngrid*sizeof(double)))) == NULL) 
    error("malloc", "out of memory gamma2grid");
  cleanDoubleVar( ngrid* ngrid, pgridremap.gamma2grid);  
...
int main(int argc, char *argv[]) {
  ...
  int option_index = 0, status, psistatus, intrp;
  int cin, nave = 4, is;
  double *gautable;
  long plotfits; // added AH - 10/18/2014
  ...
  nztot = 1;
  recon = 0;
  plotfits = 0; // Added AH - 10/18/2014
  plot = 0; // Added AH - 10/18/2014
  while (1)
    {
...
--name     name of file with filenames \n\
--psifile  name of file with psi values \n\
--ngrid    number of gridpoints (1024) \n\
--plot     plot (0=n,1=all) (0) \n\
--image    image to rec (-1 = all) \n\
--sigma    smoothing for maps (pix, 0.01)\n\
--recon    do the image mapping (0) \n\
--plotfits plot the fits files (0)" );
           exit (EXIT_SUCCESS);
           break;
        case 'd':
          ...
          break;
        case 'j':
          strcpy(my_args[1],optarg);
          psistatus += 1;
          // message("Psifile used is %s", my_args[1]);
          break;
	case 'p':
          plot = atoi(optarg);
          break;
  ...
  if (status != 1) {
   ...
   if (psistatus != 1) {
    message("psistatus = %d",psistatus);
    error("main","Psi file not specified.");
  }
  // printf("Initially, dista[0] = %f", dista[0]);
  ...
  read_psi(my_args[1]); // changed by AH on 11-05-2014 to read user-specified psi file
 ...
 /**********************************************************************/
  ...
  // z2 = 7.16;
  // zA2744 = 0.451;
  // zcos1 = redshift(z1,zrxj);
  // zcos2 = redshift(z2,zrxj);
  // message("zcosmo1 = %f, zcosmo2 = %f \n",zcos1,zcos2);
  // zcos1 = redshift(z1,plenses.z0[0]);
  // zcos2 = redshift(z2,plenses.z0[0]);
  // zcos4 = redshift(z4,plenses.z0[0]);
  // zcos9 = redshift(z9,plenses.z0[0]);
  // message("zcos1 = %f, zcos2 = %f, zcos4 = %f, zcos9 = %f ",zcos1,zcos2,zcos4,zcos9);
  // The factor you must multiply z=inf maps by to get Dds/Ds = 1 map 
  message("zcosmo(z=%f) = %f \n",zcur,zcosmo);
  message("zlens = %f \n", plenses.z0[0]);

  double angdd, angdd1, angdd2, angdd3;
  angdd = angdd_correct(0.0,1E7);
  angdd1 = angdd_correct(plenses.z0[0],1E7); 
  angdd2 = angdd_correct(0.0,pimages.zs[pimages.ncur]);
  angdd3 = angdd_correct(plenses.z0[0],pimages.zs[pimages.ncur]);
  message (" angular diameter distances DA(0,inf) -> %f DA(zl,inf) -> %f DA(0,zs) -> %f DA(zl,zs) -> %f \n ", angdd, angdd1, angdd2, angdd3);
  ...
  for (is=0;is<pimages.nsystems;is++){
	...
	theta = atan2 (-fitsheader.cd[1], fitsheader.cd[3]); 
      ...
      for (i=0;i<ngrid; i++){  
	for (j=0;j<ngrid; j++){
    // pgridremap.psigrid[i*ngrid+j] = zcosmo*pgridremap.psicomp[i*ngrid+j]; // Added by AH on 5/26/2015
	  pgridremap.kappagrid[i*ngrid+j] =  zcosmo*pgridremap.kappacomp[i*ngrid+j];
	  pgridremap.gamma1grid[i*ngrid+j] =  zcosmo*pgridremap.gamma1comp[i*ngrid+j];
	  pgridremap.gamma2grid[i*ngrid+j] =  zcosmo*pgridremap.gamma2comp[i*ngrid+j];
	  pgridremap.alpha1grid[i*ngrid+j] =  zcosmo*pgridremap.alpha1comp[i*ngrid+j];
